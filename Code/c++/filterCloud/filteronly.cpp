...
// Description : Modified to only filter based on retro-intensity
//============================================================================
...
#define THRESHOLD_RETRO 0.40
// #define THRESHOLD_RETRO 0.30
// #define DISTANCE 2
...
static void help() {
    cout
        ...
        << " \n"
        << " Usage:\n"
        << "        ./filteronly <path_to_input_LAS_or_CSV> <path_to_output_folder>\n"
        << " \n"
        << " Example:\n"
        << "        ./filteronly data/20180223/input/HD_20180223_Campus_EB_Run1_LAS1_2.las data/20180223/output/HD_20180223_Campus_EB_Run1/\n"
        << "    or:\n"
        << "        ./filteronly data/20180223/input/HD_20180223_Campus_EB_Run1.csv data/20180223/output/HD_20180223_Campus_EB_Run1/\n"
        << "------------------------------------------------------------------\n\n"
        << endl;
}
...
void getPointsFromCSV(string filename, map<int, PointCld >& imap, const string& mode)
{
        ...
        if(k>0) // ignore csv header
          {
              ...
              for (int i=0; getline(line_ss, cell, ','); i++)
              {
              	...
              	if (mode == "normalized")
              	{
              		// format for normalized point cloud (output of normalize)
                  if(i==0) point.id = k;
                  if(i==2) point.y = stod(cell); // LAT
                  if(i==1) point.x = stod(cell); // LONG
                  if(i==3) point.z = stod(cell);
              	...
              	{
              		// format for original point cloud
                  if(i==0) point.id = k;
                  if(i==2) point.y = stod(cell); // LAT
                  if(i==1) point.x = stod(cell); // LONG
                  if(i==3) point.z = stod(cell);
...
void writeOutCSV(string folderName, string filename, map<int, PointCld>& filteredMap)
{
    ...
    std::string outputCSV = folderName + "/" + filename;
    ofstream csv;
    csv.open(outputCSV.c_str());

    csv << "SignId,Id,X,Y,Z,Retro,angle,distance,UTC\n";
    for (map<int, PointCld>::iterator it=filteredMap.begin(); it!=filteredMap.end(); ++it)
    {
        PointCld point = it->second;
        csv << point.signId << "," << point.id << "," << fixed << setprecision(7) << point.x << "," << point.y << "," << setprecision(3) << point.z
            << "," << setprecision(2) << point.retro << "," << setprecision(5) << point.angle << ","<< setprecision(3) << point.dist << ","<< setprecision(3) << point.utc  << "\n";
...
int main(int argc, char** argv) {
    ...
    try
    {
        ...
        {
            getPointsFromCSV(inputFile, imap, "raw");
            // getPointsFromCSV(inputFile, imap, "normalized");
        ...
        writeOutCSV(folderName, "signFilter.csv", filteredMap);
